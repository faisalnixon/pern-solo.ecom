 ...
 */
export type ImageKitCropMode = "at_max" | "maintain_ratio";
...
function buildpernsoloecomTextLayer({ w, h }: TextLayerOpts): string {
  ...
  return `l-text,i-pern-solo.ecom,fs-${fs},co-FFFFFF,bg-0F172A90,pa-8_12,lx-N14,ly-14,lap-top_right,l-end`; // Creates an ImageKit transformation that overlays the text "Northwind" onto the image. The overlay uses a white font, semi-transparent dark background, dynamic font size, padding, and positions the text at the top-right corner.i-pern-solo.ecom  Overlay text is "pern-solo.ecom"
}
...
function buildTrSegment({
  ...
  parts.push(`f-${f}`);
  const base = `tr:${parts.join(",")}`; // "tr" stands for transformation. It tells ImageKit that the following values (width, height, quality, format, etc.) are image transformation parameters.
  if (!watermark) return base;
  return `${base}:${buildpernsoloecomTextLayer({ w, h })}`; //This tells ImageKit to first resize/optimize the image, then add the text watermark.
}
