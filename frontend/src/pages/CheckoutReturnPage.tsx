...
function CheckoutReturnPage() {
  ...
  const queryClient = useQueryClient(); // useQueryClient() gives access to React Query's global cache and query manager. // It lets you refresh, update, or remove cached query data (e.g., invalidateQueries() to refetch fresh data).

  useEffect(() => {
    clearCart();
    queryClient.invalidateQueries({ queryKey: ["orders"] });// Tells React Query that any cached data matching the ["orders"] key is outdated.  // Triggers an immediate refetch for active order queries so the user sees their new purchase.
