...
const tabClass = ({ isActive }: NavLinkRenderProps) => `tab gap-2 whitespace-nowrap ${isActive ? "tab-active" : ""}`;
//NavLinkRenderProps provides info about a NavLink's state. NavLinkRenderProps is the object React Router passes to NavLink's className/style callback. It includes isActive (true if the current URL matches the link, otherwise false), e.g. "/orders/123" → Summary link: isActive=true, Chat link: isActive=false. It includes isActive (true if the current URL matches the link, otherwise false), e.g. "/orders" => true for <NavLink to="/orders">.
    ...
    <div className="space-y-8 text-left">
      ...
      <div>
        ...
        <div className="tabs tabs-boxed mt-3 w-fit flex-wrap bg-base-300/50 p-1">
          ...
          {/* // `end` makes NavLink active only when the URL matches exactly. Example: with `end`, "/orders/123" is active only on "/orders/123", not "/orders/123/chat". */}
        ...
        <div className="mt-5">
          ...
          {/* // useOutletContext() lets a child route access data passed from its parent route via <Outlet context={...}>. Example: OrderDetailPage passes { order, items }, and OrderSummaryPage reads them without props. */}
