  ...
  try {
    ...
    if (orderIds.length > 0) {
      ...
      for (const row of itemRows) {
        const list = previewByOrder.get(row.orderId) ?? []; // previewByOrder.get(row.orderId)  this will make a array of all the items in the order by the orderId and if there is no orderId then it will return an empty array
        list.push({
          name: row.name,
          slug: row.slug,
    ...
    const ordersPayload = rows.map((o) => ({
      ...
      // get will return the array of items for the orderId and if there is no orderId then it will return an empty array
    }));
...
export async function createVideoInvite(
  ...
  try {
    ...
    const customerSid = streamUserId(owner.clerkUserId); // customerSid = customer stream id
    await server.upsertUser({
      id: customerSid,
