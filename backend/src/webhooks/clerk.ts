...
export async function clerkWebhookHandler(req: Request, res: Response) {
  ...
  try {
    ...
    if (evt.type === "user.created" || evt.type === "user.updated") {
      ...
      // onConflictDoUpdate used to update the user if it already exists in the database, instead of throwing an error or ignoring the insert this mwthod will update the user with the new values if it already exists in the database
    }
